...
public:
    ...
    bool pos_komi() {
        return (get_to_move() == FastBoard::BLACK && (m_opp_komi > 7.5 || m_komi > 7.5)) || (get_to_move() == FastBoard::WHITE && (m_opp_komi < -7.5 || m_komi < -7.5));
    }
    bool neg_komi() {
        return (get_to_move() == FastBoard::BLACK && (m_opp_komi < -7.5 || m_komi < -7.5)) || (get_to_move() == FastBoard::WHITE && (m_opp_komi > 7.5 || m_komi > 7.5));
    }
    bool eval_invalid() {
        return (pos_komi() && cfg_neg) || (neg_komi() && cfg_pos);
    }
