...
std::pair<int, int> Network::load_network(std::istream& wtfile,
    ...
    const auto ending_weights = size_t{14};
    if (version == 3) {
        input_weights = 5;
    ...
    const auto plain_conv_wts =
        input_weights + residual_blocks * residual_block_weights;
    linecount = 0;
    while (std::getline(wtfile, line)) {
    ...
    for (auto i = size_t{0}; i < bias_size; i++) {
        ...
        for (auto j = size_t{0}; j < means_size; j++) {
            ...
            m_fwd_weights->m_batchnorm_means[i][j] *=
                m_fwd_weights->m_batchnorm_gammas[i][j];
            m_fwd_weights->m_batchnorm_means[i][j] -=
                m_fwd_weights->m_batchnorm_betas[i][j]
